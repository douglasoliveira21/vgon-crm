...
func GetWidgetEmbedScript(svc *services.Container) fiber.Handler {
  ...
  fetch(apiBase+"/api/widget/"+widgetId+"/config").then(function(r){return r.json();}).then(function(cfg){
    ...
    panel.querySelector("#vgon-start").onclick=function(){
      ...
      setScreen("chat");
      var log=document.getElementById("vgon-log");
      if(log&&log.childNodes.length===0){appendMsg(log,(cfg.greeting_message||"Olá! 👋\nComo podemos ajudar você hoje?"),false,color,"welcome-"+Date.now(),"","",null,"text","");}
    };
