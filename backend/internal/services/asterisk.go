...
func (s *AsteriskService) AMIReloadPJSIP(companyID string) error {
	return s.ReloadPJSIP(companyID)
}

func (s *AsteriskService) GeneratePJSIPTrunkConfig(id, name, server string, port int, transport, domain, username, password, callerID, realm, outboundProxy string, codecs []string, nat bool, keepAlive int, dtmf string, registerExpires int) string {
	if port == 0 {
		port = 5060
	}
	if transport == "" {
	...
	section := "trunk-" + sanitizePJSIPName(id)
	endpoint := []string{
		"; Generated by EvoCRM. Do not edit manually.",
		"[" + section + "]",
		"type=endpoint",
		"transport=transport-" + strings.ToLower(transport),
		"context=from-trunk",
		"disallow=all",
		"allow=" + strings.Join(codecs, ","),
	...
	if callerID != "" {
		endpoint = append(endpoint, "from_user="+callerID)
	}
	if nat {
		endpoint = append(endpoint, "rtp_symmetric=yes", "force_rport=yes", "rewrite_contact=yes")
	}
	...
	if realm != "" {
		cfg = append(cfg, "realm="+realm)
	}
	cfg = append(cfg,
		"",
		"["+section+"-aor]",
		"type=aor",
		"contact=sip:"+server+":"+fmt.Sprintf("%d", port),
		"qualify_frequency="+fmt.Sprintf("%d", keepAlive),
	)
	if outboundProxy != "" {
		cfg = append(cfg, "outbound_proxy="+outboundProxy)
	}
	if username != "" && password != "" {
		cfg = append(cfg,
			...
			"type=registration",
			"outbound_auth="+section+"-auth",
			"server_uri=sip:"+server+":"+fmt.Sprintf("%d", port),
			"client_uri=sip:"+username+"@"+domain,
			"retry_interval=60",
			"expiration="+fmt.Sprintf("%d", registerExpires),
		)
	}
	return strings.Join(cfg, "\n") + "\n"
}

func (s *AsteriskService) ReloadPJSIP(companyID string) error {
	var host, user, password string
	var port int
	err := s.db.QueryRow(`
		SELECT COALESCE(ami_host, ''), COALESCE(ami_port, 5038), COALESCE(ami_user, ''), COALESCE(ami_password, '')
		FROM telephony_providers
		WHERE company_id = $1 AND is_active = true
		LIMIT 1
	`, companyID).Scan(&host, &port, &user, &password)
	if err != nil || host == "" || user == "" {
		log.Printf("[ASTERISK] PJSIP reload skipped: AMI not configured for company %s", companyID)
		return nil
	}
	...
	for _, command := range commands {
		if _, err := conn.Write([]byte(command)); err != nil {
			return fmt.Errorf("AMI command failed: %w", err)
		}
	}
...
func sanitizePJSIPName(value string) string {
